<?php

namespace Restfull\Filesystem;

use DirectoryIterator;

/**
 * Class Folder
 * @package Restfull\Filesystem
 */
class Folder
{

    /**
     * @var string
     */
    private $path = '';

    /**
     * @var array
     */
    private $info = [];

    /**
     * @var bool
     */
    private $exist = false;

    /**
     * Folder constructor.
     * @param string|null $folder
     */
    public function __construct(string $folder = null)
    {
        if (!isset($folder)) {
            $this->path = ROOT_PATH;
        } else {
            $path = '';
            if (substr($folder, 0, strlen("Restfull")) == "Restfull") {
                $path = RESTFULL;
                $folder = substr($folder, strlen("Restfull") + 1);
            }
            $this->path = $path . $folder;
        }
        return $this;
    }

    /**
     * @param string|null $path
     * @param int $mode
     * @return bool
     */
    public function create(string $path = null, int $mode = 0755): bool
    {
        if (is_null($path)) {
            $path = $this->path();
        } else {
            $path = $this->path() . $path;
        }
        if (!is_dir($path)) {
            if (count($this->info) == 0) {
                $this->info($path);
            }
            $create = true;
            if (stripos($this->info['basename'], DS)) {
                $create = $this->create($this->info['basename']);
            }
            if ($create) {
                if ($this->exists()) {
                    umask(0);
                    $success = mkdir($this->info['filename'], $mode, true) !== false ? true : false;
                    umask(0);
                }
                return $success;
            }
        }
        return false;
    }

    /**
     * @return string
     */
    public function path(): string
    {
        return $this->path;
    }

    /**
     * @param string $path
     * @return Folder
     */
    public function info(string $path): Folder
    {
        $this->info = pathinfo($path);
        return $this;
    }

    /**
     * @return bool
     */
    public function exists(): bool
    {
        return file_exists($this->path);
    }

    /**
     * @param string|null $path
     * @return Folder
     */
    public function delete(string $path = null): Folder
    {
        if (is_null($path)) {
            $path = $this->path();
        } else {
            if (is_dir($path) && empty($path)) {
                $path = $this->path() . DS . $path;
            }
        }
        $filesDirs = $this->read($path);
        $this->info($path);
        if (empty($filesDirs['diretory']) && empty($filesDirs['file'])) {
            rmdir($this->info['filename']);
        }
        if ($this->exists()) {
            if (isset($filesDirs['diretory'])) {
                for ($a = 0; $a < count($filesDirs['diretory']); $a++) {
                    $this->delete($filesDirs['diretory'][$a]);
                }
            }
            if (isset($filesDirs['file'])) {
                for ($a = 0; $a < count($filesDirs['file']); $a++) {
                    $this->delete($filesDirs['file'][$a]);
                }
            }
            if (empty($filesDirs['diretory']) && empty($filesDirs['file'])) {
                rmdir($this->info['filename']);
            }
        }
        $this->exist = $this->exists();
        return $this;
    }

    /**
     * @param string|null $path
     * @return array
     */
    public function read(string $path = null): array
    {
        if (is_null($path)) {
            $path = $this->path();
        } else {
            if (is_dir($path) && empty($path)) {
                $path = $this->path() . DS . $path;
            }
        }
        $read = new DirectoryIterator($path);
        foreach ($read as $item) {
            if ($item->isDot()) {
                continue;
            }
            $reading = $item->getFilename();
            if ($reading != 'empty') {
                if ($item->isDir()) {
                    $dirs[] = $reading;
                } else {
                    $files[] = $reading;
                }
            }
        }
        if (isset($dirs)) {
            return ['diretory' => $dirs];
        } elseif (isset($files)) {
            return ['file' => $files];
        } elseif (isset($dirs) && isset($files)) {
            return ['diretory' => $dirs, 'file' => $files];
        }
        return ['diretory' => [], 'file' => []];
    }

    /**
     * @return bool
     */
    public function issets(): bool
    {
        return $this->exist;
    }
}