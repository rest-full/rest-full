...
class TwoFactorComponente extends Component
{
    ...
    /**
     * TwoFactorComponente constructor.
    ...
    {
        $query = $this->controller->querys('all', ['table' => $table], ['query' => 'show columns from %s'], false);
        $valid = false;
        ...
        $query = $this->controller->querys(
                'first',
                ['table' => $table],
                ['fields' => ['twofactor'], 'conditions' => $user],
                false
        );
        return $query->twofactor;
    }
