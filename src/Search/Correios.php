...
class Correios
{
     ...
     * @throws Exceptions
     */
    public function cep(string $cep, bool $ajax = false): Correios
    {
        ...
        $answer = $this->search->zipcode()->find(str_replace("-", "", $cep));
        if (isset($answer['error']) && $answer['error'] == 'CEP INVÁLIDO') {
            if ($ajax) {
                $this->data = [
                        'status' => 'Dados não encontrado',
                        'result' => '',
                        'address' => '',
                        'district' => '',
                        'city' => '',
            ...
            throw new Exceptions('CEP não encontrado');
        }
        $newStreet = '';
    ...
    private function validate(string $cep): array
    {
        if (strlen(str_replace("-", "", $cep)) < 8) {
            return ['valid' => true, 'resp' => 'CEP menor que 8 caracteres'];
        }

        if (!preg_match('/^[0-9]{5}\-[0-9]{3}$/i', $cep)) {
            return ['valid' => true, 'resp' => 'CEP inválido.'];
        }
